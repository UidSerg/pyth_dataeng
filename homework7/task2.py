...
def rename_group(new_name = "New", num = 2, extension="txt", extension_new="doc", old_name=[1,5]):
    """Функция переименовывает группу файлов.
    
    new_name - желаемое новое имя
    num - колличество знаков в числе
    """
        ...
        if obj.is_file():
            if obj.name.split('.')[-1] == extension:
                if extension not in no_rename_extension:
                    Path(obj).rename(f'{obj.name.split('.')[0][old_name[0]:old_name[1]]}_{new_name}_{str(i).zfill(num)}.{extension_new}')
                    i += 1
                else:
